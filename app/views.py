...
MARIO_PERSPECTIVES = {
    ...
    "bowsermover": {
        "verbose": "BowserMover actúa como el motor físico invisible (controlador lógico espacial) para Bowser. Opera en segundo plano gestionando las coordenadas de movimiento, las rutas y las detecciones de colisión con el laberinto. Al separar el cálculo direccional de la representación visual, permite que el sprite visible de Bowser se centre únicamente en animar sus disfraces y reaccionar a la vulnerabilidad. Coordina mediante variables de posición el lugar exacto que el enemigo principal debe adoptar en cada fotograma.",
        "schematic": "<ul><li>Actúa como el motor físico invisible que calcula las coordenadas y rutas de Bowser por el laberinto.</li><li>Libera al sprite visual de Bowser de la carga de procesar colisiones y límites con el entorno.</li><li>Actualiza continuamente variables de posición que el enemigo principal lee para ubicarse gráficamente en pantalla.</li><li>Garantiza que el desplazamiento del jefe final sea fluido y respete las reglas espaciales estructurales del escenario.</li></ul>"
    },
    "title_page": {
        "verbose": "TitlePage gestiona la primera toma de contacto del jugador con el juego, actuando como el controlador del estado inicial ('Menú' o 'Pantalla de Título'). Presenta la interfaz gráfica de bienvenida y mantiene el sistema en pausa a la espera de la interacción del usuario. Su ocultación marca la transición sistémica del estado de espera al estado de juego activo, coordinándose a menudo con broadcasts iniciales que despiertan al resto de sprites para que comiencen sus rutinas y comportamientos.",
        "schematic": "<ul><li>Controla el estado inicial del sistema presentando la pantalla de bienvenida o título al jugador.</li><li>Mantiene la lógica del juego en un estado de espera hasta detectar la interacción que inicia la partida.</li><li>Se oculta automáticamente para ceder el control visual y el foco de la acción al entorno jugable del laberinto.</li><li>Marca la transición del sistema desde el menú hacia la activación de las rutinas de movimiento del resto de sprites.</li></ul>"
    }
}
