...
MARIO_PERSPECTIVES = {
    "Mario": {
        "verbose": "Mario se muestra y se mueve siguiendo las instrucciones de MarioMover, adaptando su posición y apariencia según la dirección y acciones del jugador[cite: 157]. Responde a eventos como inicio de nivel, colisiones con enemigos y recolección de objetos, ajustando su visibilidad y animaciones[cite: 157]. Su interacción con variables de estado determina cuándo es vulnerable, cuándo puede atacar y cómo se refleja su puntuación y posición en el escenario[cite: 157]. La coordinación con sonidos y broadcasts asegura que su comportamiento sea coherente dentro de la dinámica general del juego[cite: 157].",
        "schematic": "<ul><li>Sigue las instrucciones de MarioMover para ajustar su posición y movimiento [cite: 157].</li><li>Se mueve y orienta según la dirección que marca el jugador, ajustando su posición en el escenario [cite: 157].</li><li>Cambia sus animaciones y apariencia basándose en las acciones realizadas y su dirección [cite: 157].</li><li>Gestiona su visibilidad, ocultándose o mostrándose según eventos de inicio de nivel y colisiones [cite: 157].</li><li>Detecta colisiones con enemigos y recoge objetos para determinar vulnerabilidad y ataques mediante variables de estado [cite: 157].</li><li>Sincroniza su comportamiento con broadcasts y efectos de sonido para coordinarse con el resto del juego[cite: 157].</li></ul>"
    },
    "MarioMover": {
        "verbose": "MarioMover gestiona la lógica que determina hacia dónde se mueve Mario y cómo cambia su orientación[cite: 158]. Establece variables como la dirección deseada y la que actualmente enfrenta Mario, responde a eventos de inicio y de colisión, y coordina broadcasts como PipeWarped para sincronizar movimientos con otros sprites[cite: 158, 159]. También controla velocidades y modifica efectos visuales temporales[cite: 159]. Esta perspectiva refleja cómo la entrada del jugador se traduce en movimiento y posición dentro del escenario[cite: 159].",
        "schematic": "<ul><li>Controla la dirección y orientación determinando hacia dónde se debe mover Mario [cite: 158].</li><li>Gestiona variables clave como la dirección deseada por el jugador y la dirección actual [cite: 158].</li><li>Responde a eventos de movimiento como el inicio de nivel y las colisiones con el entorno [cite: 158].</li><li>Utiliza broadcasts como PipeWarped para sincronizar su estado con otros sprites [cite: 158].</li><li>Ajusta la velocidad y aplica cambios temporales en los efectos visuales del personaje [cite: 159].</li><li>Traduce la entrada directa del jugador en un movimiento físico dentro del escenario[cite: 159].</li></ul>"
    },
    "Coin": {
        "verbose": "Coin y MainStars funcionan como elementos coleccionables que estructuran la recompensa y el avance del jugador[cite: 163]. Su aparición y visibilidad se regulan mediante variables de estado y eventos del juego[cite: 163]. Al ser recogidos por Mario, desencadenan efectos visuales, sonidos y actualizaciones de puntuación o progreso[cite: 163, 164]. Su comportamiento refuerza la exploración del escenario y orienta los objetivos del jugador[cite: 164]. La coordinación con broadcasts garantiza que su recogida influya en otros sistemas, integrándose en el equilibrio entre desafío, motivación y ritmo de la partida[cite: 164].",
        "schematic": "<ul><li>Funcionan como objetos coleccionables que estructuran la recompensa principal [cite: 163].</li><li>Regulan su aparición y visibilidad a través de variables y eventos del estado del juego [cite: 163].</li><li>Interactúan con Mario al ser recogidos para actualizar la puntuación y el progreso del jugador [cite: 163].</li><li>Desencadenan efectos visuales y sonoros que actúan como feedback inmediato al jugador [cite: 163, 164].</li><li>Motivan la exploración del escenario y definen los objetivos a corto plazo [cite: 164].</li><li>Sincronizan su recogida con broadcasts para provocar cambios de nivel y alterar el ritmo del juego[cite: 164].</li></ul>"
    },
    "MainStars": { 
        "verbose": "Coin y MainStars funcionan como elementos coleccionables que estructuran la recompensa y el avance del jugador[cite: 163]. Su aparición y visibilidad se regulan mediante variables de estado y eventos del juego[cite: 163]. Al ser recogidos por Mario, desencadenan efectos visuales, sonidos y actualizaciones de puntuación o progreso[cite: 163, 164]. Su comportamiento refuerza la exploración del escenario y orienta los objetivos del jugador[cite: 164]. La coordinación con broadcasts garantiza que su recogida influya en otros sistemas, integrándose en el equilibrio entre desafío, motivación y ritmo de la partida[cite: 164].",
        "schematic": "<ul><li>Funcionan como objetos coleccionables que estructuran la recompensa principal [cite: 163].</li><li>Regulan su aparición y visibilidad a través de variables y eventos del estado del juego [cite: 163].</li><li>Interactúan con Mario al ser recogidos para actualizar la puntuación y el progreso del jugador [cite: 163].</li><li>Desencadenan efectos visuales y sonoros que actúan como feedback inmediato al jugador [cite: 163, 164].</li><li>Motivan la exploración del escenario y definen los objetivos a corto plazo [cite: 164].</li><li>Sincronizan su recogida con broadcasts para provocar cambios de nivel y alterar el ritmo del juego[cite: 164].</li></ul>"
    ...
    "Browser": {
        "verbose": "Bowser se comporta como un enemigo autónomo que se desplaza por el escenario siguiendo patrones de movimiento y reglas de colisión[cite: 159]. Su posición se actualiza mediante un sprite controlador, mientras su representación visual ajusta disfraces y efectos gráficos[cite: 159]. Reacciona a variables de estado que determinan si puede dañar a Mario o ser derrotado, modificando colores y activando recompensas cuando corresponde[cite: 159]. Las colisiones desencadenan reinicios de posición o eventos de GameOver[cite: 159]. La coordinación con broadcasts y sonidos asegura que su comportamiento se integre en la dinámica general[cite: 159].",
        "schematic": "<ul><li>Actúa como un enemigo autónomo que se desplaza mediante patrones de movimiento definidos [cite: 159].</li><li>Delega la gestión de su posición al controlador BowserMover para ajustarse al escenario [cite: 159].</li><li>Cambia continuamente de disfraces y modifica sus efectos gráficos según su estado actual [cite: 159].</li><li>Utiliza variables para determinar sus reglas de estado, vulnerabilidad o nivel de peligro [cite: 159].</li><li>Gestiona colisiones con Mario que resultan en daño al jugador o en la entrega de recompensas [cite: 159].</li><li>Activa reinicios de posición o eventos de GameOver coordinándose mediante broadcasts y sonidos[cite: 159].</li></ul>"
    },
    "RedBrowser": {
        "verbose": "Bowser se comporta como un enemigo autónomo que se desplaza por el escenario siguiendo patrones de movimiento y reglas de colisión[cite: 159]. Su posición se actualiza mediante un sprite controlador, mientras su representación visual ajusta disfraces y efectos gráficos[cite: 159]. Reacciona a variables de estado que determinan si puede dañar a Mario o ser derrotado, modificando colores y activando recompensas cuando corresponde[cite: 159]. Las colisiones desencadenan reinicios de posición o eventos de GameOver[cite: 159]. La coordinación con broadcasts y sonidos asegura que su comportamiento se integre en la dinámica general[cite: 159].",
        "schematic": "<ul><li>Actúa como un enemigo autónomo que se desplaza mediante patrones de movimiento definidos [cite: 159].</li><li>Delega la gestión de su posición al controlador BowserMover para ajustarse al escenario [cite: 159].</li><li>Cambia continuamente de disfraces y modifica sus efectos gráficos según su estado actual [cite: 159].</li><li>Utiliza variables para determinar sus reglas de estado, vulnerabilidad o nivel de peligro [cite: 159].</li><li>Gestiona colisiones con Mario que resultan en daño al jugador o en la entrega de recompensas [cite: 159].</li><li>Activa reinicios de posición o eventos de GameOver coordinándose mediante broadcasts y sonidos[cite: 159].</li></ul>"
    ...
    "BlueEnemy": {
        "verbose": "Los enemigos de colores se comportan como entidades autónomas que recorren el escenario siguiendo patrones de movimiento definidos por variables de dirección y sistemas de detección de choques[cite: 160]. Sus sprites controladores gestionan posiciones y giros, mientras los sprites visibles actualizan disfraces y efectos gráficos[cite: 160]. Reaccionan a cambios de estado que determinan si representan una amenaza o pueden ser derrotados, alterando su apariencia visual[cite: 160]. Las colisiones provocan reinicios, recompensas o GameOver[cite: 160]. La coordinación mediante broadcasts, temporización y efectos visuales garantiza que su comportamiento sea consistente y equilibrado[cite: 160].",
        "schematic": "<ul><li>Actúan como entidades autónomas desplazándose continuamente por el escenario [cite: 160].</li><li>Siguen patrones de movimiento y giros determinados por sistemas de detección de choques [cite: 160].</li><li>Delegan el control de movimiento y posición en sprites controladores auxiliares [cite: 160].</li><li>Actualizan sus disfraces y modifican efectos gráficos según su estado de amenaza o vulnerabilidad [cite: 160].</li><li>Provocan reinicios, entregan recompensas o activan el GameOver al colisionar con Mario [cite: 160].</li><li>Se coordinan mediante broadcasts y temporización para mantener la consistencia en la jugabilidad[cite: 160].</li></ul>"
    },
    "GreenEnemy": {
        "verbose": "Los enemigos de colores se comportan como entidades autónomas que recorren el escenario siguiendo patrones de movimiento definidos por variables de dirección y sistemas de detección de choques[cite: 160]. Sus sprites controladores gestionan posiciones y giros, mientras los sprites visibles actualizan disfraces y efectos gráficos[cite: 160]. Reaccionan a cambios de estado que determinan si representan una amenaza o pueden ser derrotados, alterando su apariencia visual[cite: 160]. Las colisiones provocan reinicios, recompensas o GameOver[cite: 160]. La coordinación mediante broadcasts, temporización y efectos visuales garantiza que su comportamiento sea consistente y equilibrado[cite: 160].",
        "schematic": "<ul><li>Actúan como entidades autónomas desplazándose continuamente por el escenario [cite: 160].</li><li>Siguen patrones de movimiento y giros determinados por sistemas de detección de choques [cite: 160].</li><li>Delegan el control de movimiento y posición en sprites controladores auxiliares [cite: 160].</li><li>Actualizan sus disfraces y modifican efectos gráficos según su estado de amenaza o vulnerabilidad [cite: 160].</li><li>Provocan reinicios, entregan recompensas o activan el GameOver al colisionar con Mario [cite: 160].</li><li>Se coordinan mediante broadcasts y temporización para mantener la consistencia en la jugabilidad[cite: 160].</li></ul>"
    ...
    "Bomb": {
        "verbose": "La bomba funciona como un elemento interactivo que aparece en momentos determinados, regulada por temporización y condiciones del juego[cite: 162]. Su animación de tamaño y visibilidad indica su estado activo, generando tensión y anticipación[cite: 162]. Al entrar en contacto con Mario, desencadena efectos visuales, sonidos y eventos que alteran el estado del juego[cite: 162]. Su comportamiento se coordina con variables y broadcasts, integrándose en la dinámica general como un mecanismo de riesgo que influye en el ritmo, la estrategia del jugador y la progresión de la partida[cite: 162].",
        "schematic": "<ul><li>Funciona como un objeto interactivo cuya aparición está estrictamente regulada por la temporización y las condiciones del juego [cite: 162].</li><li>Utiliza animaciones de tamaño y cambios de visibilidad para indicar visualmente su estado de activación [cite: 162].</li><li>Genera tensión y anticipación en el jugador al mostrarse en momentos estratégicos [cite: 162].</li><li>Desencadena efectos visuales, reproduce sonidos y modifica el estado del juego al colisionar con Mario [cite: 162].</li><li>Actúa como un mecanismo de riesgo que influye directamente en el ritmo y la estrategia de la partida[cite: 162].</li></ul>"
    },
    "MazeChanger": {
        "verbose": "MazeChanger regula los cambios del entorno, controlando cuándo y cómo se modifican los fondos para reflejar la progresión del juego[cite: 163]. Gestiona transiciones entre laberintos, asegurando que los sprites mantengan coherencia espacial tras eventos como avance de nivel o reinicios[cite: 163]. Estas dinámicas influyen indirectamente en todos los personajes, ya que redefinen el espacio de interacción[cite: 163]. La coordinación mediante broadcasts, temporización y variables garantiza continuidad visual y estructural en la experiencia de juego[cite: 163].",
        "schematic": "<ul><li>Regula los cambios del entorno controlando la transición visual entre los diferentes laberintos del juego [cite: 163].</li><li>Asegura la continuidad espacial reubicando o manteniendo la coherencia de las posiciones de los sprites tras avanzar de nivel o reiniciar [cite: 163].</li><li>Redefine el espacio de interacción, influyendo de manera indirecta en el comportamiento de todos los personajes [cite: 163].</li><li>Sincroniza las transiciones utilizando broadcasts, variables y temporización para mantener la estructura visual[cite: 163].</li></ul>"
    },
    "Backdrips": {
        "verbose": "Backdrips actúa como soporte visual, mostrando los escenarios correctos y sincronizando efectos gráficos asociados a cada estado del juego[cite: 163]. Estas dinámicas influyen indirectamente en todos los personajes, ya que redefinen el espacio de interacción[cite: 163]. La coordinación mediante broadcasts, temporización y variables garantiza continuidad visual y estructural en la experiencia de juego[cite: 163].",
        "schematic": "<ul><li>Actúa como soporte visual mostrando los escenarios correctos correspondientes a cada nivel [cite: 163].</li><li>Aplica efectos gráficos específicos que están asociados directamente al estado actual del juego [cite: 163].</li><li>Redefine el entorno visual de interacción, influyendo de manera indirecta en la percepción de los personajes [cite: 163].</li><li>Mantiene la continuidad estética coordinando sus cambios mediante variables y temporización[cite: 163].</li></ul>"
    },
    "Score": {
        "verbose": "Score controla la representación y actualización de la puntuación del jugador a medida que se recogen monedas, estrellas o se completan objetivos[cite: 164]. Maneja la creación de clones para mostrar dígitos en pantalla, ajustando posición y tamaño para mantener claridad visual[cite: 164]. Responde a eventos como inicio de nivel o GameOver, mostrando el puntaje actual y permitiendo su re-dibujo en pantallas de fin de partida[cite: 164]. Su coordinación con broadcasts y variables asegura que la puntuación refleje correctamente el progreso y logros del jugador[cite: 164].",
        "schematic": "<ul><li>Gestiona la actualización y el almacenamiento continuo de la puntuación en base a las variables de monedas y estrellas recolectadas [cite: 164].</li><li>Utiliza la creación de clones para representar visualmente cada dígito numérico en la pantalla [cite: 164].</li><li>Ajusta dinámicamente la posición y el tamaño de los clones para garantizar la claridad visual del marcador [cite: 164].</li><li>Responde a eventos globales como el inicio de nivel y el GameOver para actualizar o redibujar el puntaje [cite: 164].</li><li>Sincroniza la visualización de la puntuación mediante variables y broadcasts para reflejar con precisión el progreso del jugador[cite: 164].</li></ul>"
    },
    "High": {
        "verbose": "High muestra información de progreso y récords al jugador[cite: 164]. Se encarga de representar puntuaciones máximas o récords históricos, ajustando tamaño, posición y visibilidad según el contexto de juego[cite: 164, 165]. Responde a eventos como inicio de partida o GameOver, coordinándose con broadcasts y variables para asegurar que los valores reflejados sean precisos[cite: 165]. Esto ayuda al jugador a evaluar constantemente sus logros y tener una referencia de desempeño dentro de la partida[cite: 165].",
        "schematic": "<ul><li>Actúa como un indicador de progreso mostrando los récords históricos de puntuación máxima [cite: 164].</li><li>Ajusta su tamaño, posición y visibilidad en función del contexto actual, como el inicio del juego o la pantalla de GameOver [cite: 164].</li><li>Garantiza la precisión de los valores mostrados sincronizándose con las variables internas y los broadcasts de actualización [cite: 165].</li><li>Permite al jugador tener una referencia clara de su mejor desempeño obtenido durante partidas anteriores[cite: 165].</li></ul>"
    },
    "Level": {
        "verbose": "Level indica el nivel actual de la partida, actualizándose al inicio de cada etapa mediante disfraces para mostrar información de progreso al jugador[cite: 164]. Ajusta su tamaño, posición y visibilidad según el contexto de juego, y responde a eventos como el inicio de la partida[cite: 164, 165]. La coordinación con broadcasts y variables asegura que los valores reflejados sean precisos y consistentes, ayudando al jugador a evaluar su avance dentro de la experiencia general[cite: 165].",
        "schematic": "<ul><li>Muestra visualmente el nivel actual en el que se encuentra el jugador para indicar su progreso [cite: 164].</li><li>Utiliza cambios de disfraz para actualizar el número del nivel al inicio de cada nueva etapa [cite: 164].</li><li>Modifica su posición y visibilidad basándose en el contexto general de la partida [cite: 164, 165].</li><li>Sincroniza su actualización de información utilizando broadcasts y variables de estado [cite: 165].</li><li>Refuerza la motivación del jugador permitiéndole evaluar su avance de manera clara[cite: 165].</li></ul>"
    },
    "Awesome!": {
        "verbose": "Awesome!, LevelUp2, PraiseMe y AweEpicTastic gestionan la retroalimentación visual y sonora cuando el jugador alcanza logros, recoge objetos clave o completa objetivos[cite: 165]. Cambian disfraces, aplican efectos de movimiento, transparencia y reproducen sonidos para reforzar la sensación de recompensa[cite: 165]. Coordinados mediante broadcasts y temporización, permiten animaciones secuenciales sincronizadas que destacan eventos importantes, motivando al jugador y enfatizando sus logros para lograr una experiencia más atractiva[cite: 165].",
        "schematic": "<ul><li>Gestionan la retroalimentación visual y sonora aplicándose exclusivamente cuando el jugador alcanza logros u objetivos clave [cite: 165].</li><li>Refuerzan la sensación de recompensa mediante cambios de disfraz, efectos de movimiento, transparencia y sonidos [cite: 165].</li><li>Se activan en eventos específicos como la recolección de objetos importantes, subidas de nivel o la obtención de puntajes extra [cite: 165].</li><li>Utilizan broadcasts y temporización para ejecutar secuencias de animación fluidas y perfectamente sincronizadas [cite: 165].</li><li>Enfatizan los logros para motivar al jugador y enriquecer significativamente la experiencia lúdica[cite: 165].</li></ul>"
    },
    "LevelUp2": {
        "verbose": "Awesome!, LevelUp2, PraiseMe y AweEpicTastic gestionan la retroalimentación visual y sonora cuando el jugador alcanza logros, recoge objetos clave o completa objetivos[cite: 165]. Cambian disfraces, aplican efectos de movimiento, transparencia y reproducen sonidos para reforzar la sensación de recompensa[cite: 165]. Coordinados mediante broadcasts y temporización, permiten animaciones secuenciales sincronizadas que destacan eventos importantes, motivando al jugador y enfatizando sus logros para lograr una experiencia más atractiva[cite: 165].",
        "schematic": "<ul><li>Gestionan la retroalimentación visual y sonora aplicándose exclusivamente cuando el jugador alcanza logros u objetivos clave [cite: 165].</li><li>Refuerzan la sensación de recompensa mediante cambios de disfraz, efectos de movimiento, transparencia y sonidos [cite: 165].</li><li>Se activan en eventos específicos como la recolección de objetos importantes, subidas de nivel o la obtención de puntajes extra [cite: 165].</li><li>Utilizan broadcasts y temporización para ejecutar secuencias de animación fluidas y perfectamente sincronizadas [cite: 165].</li><li>Enfatizan los logros para motivar al jugador y enriquecer significativamente la experiencia lúdica[cite: 165].</li></ul>"
    ...
    "Joystick": {
        "verbose": "El sprite Joystick permite al jugador mover a Mario de manera intuitiva en el escenario mediante interacción táctil o con el ratón[cite: 165]. Detecta la posición del puntero y la dirección del movimiento, ajustando la orientación de Mario y activando variables de velocidad y dirección[cite: 165, 166]. Su visibilidad y efectos se adaptan según la interacción, y coordinado con broadcasts, garantiza que los movimientos sean precisos y responsivos, ofreciendo control directo sobre el personaje[cite: 166, 167].",
        "schematic": "<ul><li>Proporciona una interfaz de entrada que permite controlar el movimiento de Mario mediante interacción táctil o con el ratón [cite: 165].</li><li>Detecta la posición exacta del puntero para calcular la dirección y ajustar la orientación del personaje [cite: 165].</li><li>Modifica variables internas de velocidad y dirección que determinan el desplazamiento físico en el escenario [cite: 166].</li><li>Adapta su propia visibilidad y apariencia gráfica en tiempo real basándose en la interacción activa del jugador [cite: 166].</li><li>Garantiza un control preciso y responsivo coordinando sus acciones mediante broadcasts y cálculos de distancia[cite: 167].</li></ul>"
    },
    "Stage": {
        "verbose": "El Stage y los sprites de fondo coordinan la presentación visual global del juego[cite: 195]. Cambian los backdrops según eventos, niveles y acciones del jugador, asegurando coherencia espacial y estética[cite: 196]. Aplican efectos visuales y organizan capas para resaltar personajes, objetos y animaciones[cite: 197]. Interactúan indirectamente con otros sistemas para mantener continuidad, estableciendo el contexto de la acción y reforzando la narrativa e inmersión en la partida[cite: 198].",
        "schematic": "<ul><li>Gestiona el escenario general controlando los backdrops y fondos en respuesta a los cambios de nivel y eventos [cite: 195].</li><li>Asegura la coherencia espacial y estética de la partida coordinando las transiciones visuales con el progreso [cite: 196].</li><li>Organiza las capas visuales para destacar adecuadamente a los personajes principales, enemigos y objetos interactivos [cite: 197].</li><li>Aplica filtros y efectos gráficos específicos que se adaptan al contexto actual de la acción [cite: 197].</li><li>Trabaja junto a controladores de entorno para establecer el contexto espacial, guiando la percepción y aumentando la inmersión del jugador[cite: 198].</li></ul>"
    }
}
...
def format_babia_dict(d: dict):
    ...
    for sprite_key, sprite_item in global_babia['sprites'].items():
        ...
        ai_data = MARIO_PERSPECTIVES.get(sprite_key, {
