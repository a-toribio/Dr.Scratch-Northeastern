...
def analysis_by_upload(request, skill_points: dict, upload):
    ...
    results_dict = {}  # Inicializar el diccionario de resultados
    try:
        ext_type_project = None
        # --- MODIFICACIÓN: Capturar el valor devuelto ---
        results_dict = analyze_project(request, file_name, filename_obj, ext_type_project, skill_points)
    except Exception as e:
        traceback.print_exc()
        # --- MODIFICACIÓN: Devolver un error si falla ---
        results_dict = {'Error': 'analysis_failed', 'details': str(e)}
    ...
    # Comprobamos si el análisis fue exitoso (no devolvió un error interno)
    if 'Error' not in results_dict:
        # Añadimos los metadatos que la vista 'show_dashboard' necesita para renderizar
        results_dict.update({
            'filename': zip_filename, # Usamos el nombre del archivo original
            'dashboard_mode': request.POST.get('dashboard_mode', 'Default'),
            'multiproject': False,
            'Error': 'None' # Importante para que pase la comprobación de 'Error'
        })
